...
echo "Waiting 12 seconds for leader election..."
sleep 12

# ── Step 2: Check leader ──
echo -e "${YELLOW}[2/6] Checking Raft leader election...${NC}"
for port in 8082 8083 8084; do
  STATE=$(curl -s localhost:$port/api/v1/raft/state 2>/dev/null || echo '{"error":"unreachable"}')
...
curl -s -X POST localhost:8082/api/v1/topics \
...
for i in $(seq 1 10); do
  RESPONSE=$(curl -s -X POST localhost:8082/api/v1/producer/produce \
    -H "Content-Type: application/json" \
...
curl -s "localhost:8082/api/v1/consumer/consume?topic=orders&group=demo-group&partition=0" | \
...
curl -s localhost:8082/api/v1/metrics/cluster | python -m json.tool 2>/dev/null || \
  curl -s localhost:8082/api/v1/metrics/cluster
